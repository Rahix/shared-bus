...
#[test]
fn fake_spi_device() {
    let expect = vec![spi::Transaction::write(&[0xffu8, 0xee])];
    let mut device = spi::Mock::new(&expect);
    device.write(&[0xffu8, 0xee]).unwrap();
    device.done()
}

#[test]
fn spi_manager_manual() {
    let expect = vec![spi::Transaction::write(&[0xabu8, 0xcd, 0xef])];
    let mut device = spi::Mock::new(&expect);
    let manager = shared_bus::BusManagerSimple::new(device.clone());
    let mut proxy = manager.acquire_spi();

    proxy.write(&[0xabu8, 0xcd, 0xef]).unwrap();
...
fn spi_proxy() {
    let expect = vec![
        spi::Transaction::write(&[0xabu8, 0xcd, 0xef]),
        spi::Transaction::transfer(&[0x01u8, 0x02], &[0x03u8, 0x04]),
    ];
    let mut device = spi::Mock::new(&expect);
    let manager = shared_bus::BusManagerSimple::new(device.clone());
    let mut proxy = manager.acquire_spi();
